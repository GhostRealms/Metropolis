...
/**
 * This is a class provided on Github <https://github.com/Skyost/SkIP/blob/master/src/fr/skyost/skip/SkIP.java>
 * It provides IP, weather, and time zone data. Should be self explanatory
 */
...
public class SkIP {
    ...
    public static final SkIPData getIPData(final String ip) {
        try {
            ...
            return new SkIPData(json.get(args[0]).toString(), json.get(args[1]).toString(), json.get(args[2]).toString(), json.get(args[3]).toString(), json.get(args[4]).toString(), Double.valueOf(json.get(args[5]).toString()), Double.valueOf(json.get(args[6]).toString()), json.get(args[7]).toString());
        }
